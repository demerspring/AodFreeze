...
mempool_fini()
{
	...
	if (first != NULL) {
		...
		for (p = first; p; p = p->next) {
			...
			LogErr("memtrack: memory leak detected! %s:%u (%u bytes)\n",
	...
	LogInfo("memtrack: Total memory leakage: %u bytes (%u blocks)\n", total, count);
...
mt_malloc(ULONG size, const char *file, ULONG line)
{
	...
	if (size == 0) {
		LogWarn("memtrack: mt_malloc: size == 0!\n");
		INT_3;
		return NULL;
	}
	...
	memset((char *)data + sizeof(struct prefix), 0xcc, size);		// fill by 0xcc: new

	//pd = (struct postfix *)(data->data + data->size);
	pd = (struct postfix *)((char *)data + sizeof(struct prefix) + data->size);
	...
	KeReleaseSpinLock(&guard, irql);
	//return data->data;
	return (char *)data + sizeof(struct prefix);
}
...
mt_free(void *ptr)
{
	...
	memset((char *)data + sizeof(struct prefix), 0xc9, data->size);	// fill by 0xc9: free
...
check(struct prefix *p)
{
	...
	if (p->magic != MAGIC) {
		LogErr("memtrack: check: invalid pre-magic! 0x%x\n", p);
		INT_3;
		return NULL;
	}

	//pd = (struct postfix *)(p->data + p->size);
	pd = (struct postfix *)((char *)p + sizeof(struct prefix) + p->size);

	if (pd->magic != MAGIC) {
		LogErr("memtrack: memtrack_free: invalid post-magic! 0x%x\n", pd);
		INT_3;
		return NULL;
	}

	if (p->size != pd->size) {
		LogErr("memtrack: memtracl_free: invalid post-size! 0x%x 0x%x\n", p, pd);
		INT_3;
		return NULL;
	}
